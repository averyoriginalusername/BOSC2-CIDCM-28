// Shopper mobile — shared primitives. Lilac is the primary brand color (stylist side).
...
const Button = ({ children, variant = 'primary', size = 'md', onClick, style, disabled, fullWidth, leftIcon }) => {
  const base = {
    fontFamily: "'Outfit', system-ui, sans-serif",
    fontWeight: 600, borderRadius: 999, border: 0,
    cursor: disabled ? 'not-allowed' : 'pointer',
    transition: 'all 140ms', width: fullWidth ? '100%' : 'auto',
  ...
  const variants = {
    primary:  { background: '#6B5BD3', color: '#fff' },          // lilac — stylist primary
    coral:    { background: '#EE5A5A', color: '#fff' },          // client-facing actions
    ...
    soft:     { background: '#E6DFFF', color: '#3F3294' },
  };
  return (
...
const Chip = ({ children, active, onClick, variant = 'quiet', size = 'md' }) => {
  ...
  const sizes = { sm: { padding: '6px 12px', fontSize: 12 }, md: { padding: '8px 16px', fontSize: 14 } }[size];
  return (
    <button onClick={onClick} style={{
      ...sizes, fontFamily: "'Outfit', sans-serif", fontWeight: 500,
      background: colors.bg, color: colors.fg, border: 0, borderRadius: 999, cursor: 'pointer',
    }}>{children}</button>
  );
};
...
const Icon = ({ name, size = 22, stroke = '#1A1410', strokeWidth = 1.6 }) => {
  const paths = {
    ...
    wallet: <><rect x="3" y="6" width="18" height="13" rx="2"/><path d="M16 6V4H8v2"/><circle cx="17" cy="12.5" r="1.4" fill={stroke}/></>,
...
// Bottom nav — lilac active for stylist
const BottomNav = ({ active, onChange }) => {
  const items = [
    { k: 'home',     label: 'Home' },
    { k: 'calendar', label: 'Calendar' },
    { k: 'message',  label: 'Messages' },
    { k: 'users',    label: 'Clients' },
      ...
      {items.map(it => (
        <button key={it.k} onClick={() => onChange && onChange(it.k)}
          style={{
            ...
            padding: 4, color: active === it.k ? '#6B5BD3' : '#8C8278',
          }}>
          <Icon name={it.k} size={20} stroke={active === it.k ? '#6B5BD3' : '#8C8278'} strokeWidth={1.8}/>
  ...
  <div style={{ padding: '8px 20px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', minHeight: 28 }}>
      {onBack ? (
        ...
        <button onClick={onClose} style={{ background: 'transparent', border: 0, cursor: 'pointer', padding: 4, color: '#6B5BD3' }}>
...
Object.assign(window, { Logo, Wordmark, WordmarkCaps, Button, Input, Chip, Avatar, Icon, BottomNav, ScreenHeader });
